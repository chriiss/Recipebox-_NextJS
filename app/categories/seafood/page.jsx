...
const SeaFood = () => {
    ...
    useEffect(() => {
        const fetchMealList = async () => {
            const fetchedMealList = await getMealList("Seafood")
            setSeaFood(fetchedMealList)
            setLoading(false)
        }
        fetchMealList()
    }, []);
                ...
                {loading ? (
                    ...
                    <>
                        <section className={styles.herozone}>
                            <article className={styles.herozone_bgSeaFood}>
                                <div className={styles.herozone_bgSeaFood_overlay}></div>
                                <div className={styles.herozone_bgSeaFood_content}>
                                    <h1>{dataFront.category.seaFood}</h1>
                                </div>
                            </article>
                        </section>
                        <section className={styles.categories}>
                            ...
                            <article className={styles.categories_flex}>
                                {seaFood.map((seafood) => (
                                    <div key={seafood.idMeal} className={styles.categories_flex_card}>
                                        <Image src={seafood.strMealThumb} alt={seafood.strMeal} width={300} height={200} className="img_responsive" />
                                        <p>{seafood.strMeal}</p>
                                        <div className={styles.categories_flex_card_buttons}>
                                            <MealDetails mealId={seafood.idMeal} dataFront={dataFront} />
                                            <FavoriteButton item={{ id: seafood.idMeal, name: seafood.strMeal, image: seafood.strMealThumb }} />
                                        </div>
                                    </div>
                                ))}
                            </article>
